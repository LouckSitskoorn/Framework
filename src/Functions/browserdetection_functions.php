<?php
  ...
  */
  ...
  function get_devicetype( $pv_browser_user_agent, $screenwidth, $screenheight )
  {
    $returnvalue = "desktop";
    $browser        = browser_detection("browser");
    $browser_number = browser_detection("number");

    $a_os_data  = get_os_data( strtolower($pv_browser_user_agent), $browser, $browser_number );
    $os_type    = $a_os_data[0];// os name, abbreviated
    $os_number  = $a_os_data[1];// os number or version if available

    if ($os_type == "android" || $os_type == "mac") {
      if (intval($screenwidth) > 0
      &&  intval($screenheight) > 0) {
        if (intval($screenwidth) * intval($screenheight) <= 700000) {
          $returnvalue = "phone:";
        } else {
          $returnvalue = "tablet";
        }
